...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}DS.IA v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Exeter.user.name}#{Exeter.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Exeter.user.id}   
                       {Fore.CYAN}Nitro Sniper | {Fore.GREEN}{nitro}
                       {Fore.CYAN}Cached Users: {Fore.GREEN}{len(Exeter.users)}
                       {Fore.CYAN}Servidores: {Fore.GREEN}{len(Exeter.guilds)}
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name=" DS.IA | Prefix: " + str(Exeter.command_prefix),
                         icon_url=Exeter.user.avatar_url)
        embed.set_thumbnail(url="https://media.discordapp.net/attachments/846780334414888971/846801643655135272/image.png?width=406&height=406")
        embed.add_field(name=":satellite_orbital: `HELP GENERAL`", value="Comandos Generales", inline=False)
        embed.add_field(name=":goggles: `HELP ACCOUNT`", value="Comandos Informaion de Cuentas", inline=False)
        embed.add_field(name=":goggles: `HELP TEXT`", value="Comndos de Textos", inline=False)
        embed.add_field(name=":goggles: `HELP IMAGE`", value="Comandos de Manipulacion de Imagenes", inline=False)
        embed.add_field(name=":goggles: `HELP NSFW`", value="Comandos NSFW", inline=False)
        embed.add_field(name=":goggles: `HELP MISC`", value="Comandos Miscellaneous", inline=False)
        embed.add_field(name=":goggles: `HELP NUKE`", value="Comandos de Raid", inline=False)
        ...
        embed.set_thumbnail(url="https://media.discordapp.net/attachments/846780334414888971/846801643655135272/image.png?width=406&height=406")
        embed.description = f"\uD83D\uDCB0 `Comandos Generales`\n`> help <category>` - Retorna los comandos de la Categoria \n`> uptime` - Cuanto tiempo has usado el bot \n`> prefix <prefix>` - Establece un Prefijo\n`> ping` - Retorna la Latencia del bot\n`> av <user>` - Obten el Avatar del ususario\n`> whois <user>` - Obten informacion del usuario \n`> tokeninfo <token>` - Obten Informacion de algun token \n`> copyserver` - Copia el servidor \n`> rainbowrole <role>` - Crea tu rainbow color (ratelimits)\n`> serverinfo` - Obten la informacion del servidor \n`> serverpfp` - Obten el icono del servidor \n`> banner` - Obten el Banner del servidor \n`> shutdown` - Apaga el Bot\n`> getroles` - Obten los roles del servidor"
        await ctx.send(embed=embed)
    elif str(category).lower() == "account":
        embed = discord.Embed(color=random.randrange(0x1000000), timestamp=ctx.message.created_at)
        embed.set_thumbnail(url="https://media.discordapp.net/attachments/846780334414888971/846801643655135272/image.png?width=406&height=406")
        embed.description = f"\uD83D\uDCB0 `ACCOUNT COMANDOS`\n`> ghost` - Obten un avatar Invisible\n`> pfpsteal <user>` - Roba la foto de los usuarios\n`> setpfp <link>` - Establece una imagen como tu avatar\n`> hypesquad <hypesquad>` - Establece a que Hypesquad estaras \n`> spoofcon <type> <name>` - Falsifica tu coneccion de Discord \n`> leavegroups` - Abandona todos los grupos \n`> cyclenick <text>` - Recorre tu apodo \n`> stopcyclenick` - Detiene tu apodo \n`> stream <status>` - Establece tu Streaming\n`> playing <status>` - Establece tu juego\n`> listening <status>` - Establece lo que escuchas \n`> watching <status>` - Establece lo que ves\n`> stopactivity` - Detiene tu actividad\n`> acceptfriends` - Acepta a tus amigos\n`> delfriends` - Elimina a un amigo\n`> ignorefriends` - Ignora a un amigo\n`> clearblocked` - Elimina de la lista de Bloqueados\n`> read` - Marca todos los mensajes como leídos\n`> leavegc` - Abandona el chat actual \n`> adminservers` - Todos los servidores en los que tiene permisos\n`> slotbot <on/off>` - snipes slotbots ({Exeter.slotbot_sniper})\n`> giveaway <on/off>` - snipes giveaways ({Exeter.giveaway_sniper})\n`> mee6 <on/off>` - Mensajes automaticos ({Exeter.mee6}) <#{Exeter.mee6_channel}>\n`> yuikiss <user>` - Auto kiss <@{Exeter.yui_kiss_user}> <#{Exeter.yui_kiss_channel}>\n`> yuihug <user>` - Auto hugs <@{Exeter.yui_hug_user}> <#{Exeter.yui_hug_channel}>\n`> yuistop` - Detiene los bucles"
        await ctx.send(embed=embed)
    elif str(category).lower() == "text":
        embed = discord.Embed(color=random.randrange(0x1000000), timestamp=ctx.message.created_at)
        embed.set_thumbnail(url="https://media.discordapp.net/attachments/846780334414888971/846801643655135272/image.png?width=406&height=406")
        embed.description = f"\uD83D\uDCB0 `TEXT COMANDOS`\n`> ds` - DS Logo\n`> snipehistory` - Historial de mensajes eliminados\n`> clearsnipehistory` - Borra el historia de los snipes del canal\n`> snipe` - Mira los mensajes eliminados del canal \n`> editsnipe` - Edita los mensajes eliminados \n`> msgsniper <on/off> ({Exeter.msgsniper})` - Activa o desactiva los snipe de los MD\n`> clear` - Limpia el canal con un mensaje de flood\n`> del <message>` - Elimina el mensaje mandado\n`> 1337speak <message>` - Escribe como Hacke\n`> minesweeper` - Juega el Busca mina\n`> spam <amount>` - Spamea un mensaje\n`> dm <user> <content>` - Envia un MD a algun usuario\n`> reverse <message>` - Envia un mensaje alrevez \n`> purge <amount>` - Elimina los mensajes\n`> empty` - Manda un mensaje vacio\n`> firstmsg` - Mira el 1° mensaje del canal\n`> ascii <message>` - Crea texto en ASCII\n`> wizz` - Envia un meme sobre Wizz \n`> 8ball <question>` - Meme de 8 ball\n`> slots` - Maquina de slots\n`> everyone` - Pingea Everyone con link\n`> abc` - Ciclo de abc\n`> 100` - Ciclo -100\n`> cum` - Crea una animacion\n`> 9/11` - Ataque d 9/11\n`> massreact <emoji>` - Reaccion masiva de mensajes"
        await ctx.send(embed=embed)
        ...
        embed = discord.Embed(color=random.randrange(0x1000000), timestamp=ctx.message.created_at)
        embed.set_thumbnail(url="https://media.discordapp.net/attachments/846780334414888971/846801643655135272/image.png?width=406&height=406")
        embed.description = f"\uD83D\uDCB0 `MISCELLANEOUS COMANDOS`\n`> copycat <user>` - Copia los mensajes de X usuario ({Exeter.copycat})\n`> stopcopycat` - Detiene e Copycat\n`> fakename` - Fakename de algun usuario\n`> geoip <ip>` - Localiza alguna IP\n`> pingweb <website-url>` Obten la latencia de alguna pagina\n`> anticatfish <user>` - Busca en google el avatar de algun user \n`> stealemoji` - <emoji> <name> - Roba el emoji mencionado\n`> hexcolor <hex-code>` - Obtenen el color hex\n`> dick <user>` - Devuelve el tamaño de la polla del usuario\n`> bitcoin` - Obten el precio de BTC\n`> hastebin <message>` - Postea un mensaje en Hastebin\n`> nitro` - Genera un Nitro code\n`> feed <user>` - Dale de comer \n`> tickle <user>` - Tickle user\n`> slap <user>` - Cachete a un usuario\n`> hug <user>` - Abrasa a alguien\n`> cuddle <user>` - Cuddles user\n`> smug <user>` - Smugs user\n`> pat <user>` - Acaricia a alguien\n`> kiss <user>` - Besa a alguien\n`> topic` - Envía un iniciador de conversación\n`> wyr` - Envía un ¿preferirías?\n`> gif <query>` - Busco un gif\n`> sendall <message>` - Manda un mensaje en el mencionado\n`> poll <msg: xyz 1: xyz 2: xyz>` - Crea una encuesta\n`> image <query>` - Busca imagenes\n`> hack <user>` - Hackea a alguien\n`> token <user>` - Token de algun user\n`> cat` - Cat\n`> sadcat` - SadCat\n`> dog` - Dog\n`> fox` - Fox\n`> bird` - Bird\n"
        await ctx.send(embed=embed)
...
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="ds-ia")
    for _i in range(250):
        await ctx.guild.create_role(name="ds-ia", color=RandomColor())
...
async def ping(ctx):
    ...
    message = await ctx.send(":satellite_orbital: Pinging...")
...
async def _ball(ctx, *, question):
    ...
    embed.set_thumbnail(url="https://media.discordapp.net/attachments/846780334414888971/846947005263773717/1024px-8-Ball_Pool.png?width=406&height=406")
    await ctx.send(embed=embed)
...
async def slot(ctx):
    ...
    if a == b == c:
        await ctx.send(embed=discord.Embed.from_dict(
            {"title": "Slot machine", "description": f"{slotmachine} ¡Todos los partidos, ganaste!"}))
    elif (a == b) or (a == c) or (b == c):
        await ctx.send(embed=discord.Embed.from_dict(
            {"title": "Slot machine", "description": f"{slotmachine} 2 seguidos, ¡ganaste!"}))
    else:
        await ctx.send(embed=discord.Embed.from_dict(
            {"title": "Slot machine", "description": f"{slotmachine} Sin partido, perdiste"}))
...
async def btc(ctx):
    ...
    em.set_thumbnail(url='https://cdn.pixabay.com/photo/2013/12/08/12/12/bitcoin-225079_960_720.png')
    await ctx.send(embed=em)
